...
impl PremiumSubscriptionsModel {
    ...
    pub async fn get_premium_status(
        ...
        use degen_sql::sql_builder::{SqlBuilder, SqlStatementBase, OrderingDirection};
        
        // Create SQL builder with where condition
        let mut where_params: BTreeMap<TinySafeString, Arc<dyn ToSql + Sync>> = BTreeMap::new();
        where_params.insert("public_address".try_into().unwrap(), Arc::new(wallet_address.clone()));
